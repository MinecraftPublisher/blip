...
const compile = () => fs.readFileSync('app/main.html', 'utf8')
    .replace('<Typescript />', `<script transpiled>/* automatically transpiled from typescript by bun transpiler */\n${uglify.minify(Transpiler.transformSync(fs.readFileSync('app/blip.ts', 'utf8').replace("export const blip = _blip", '') + '\n' +
    fs.readFileSync('app/app.ts', 'utf8').replace("import { blip } from './blip.js'", '').replace("blip(", "_blip("), {})).code}\n</script>`);
if (!process.argv.includes('build')) {
    Bun.serve({
        port: port,
        fetch(request) {
            if (!request.url.includes('favicon.ico'))
                console.log(`${chalk.green(chalk.bold(`get`))} ${request.url}`);
            else
                return new Response('fuck you bitch');
            ...
            fetch(process.env.CHECK_URL ?? '').then(e => e.text()).then(e => {
                if (e !== compiled) {
                    fetch((process.env.UPLOAD_URL ?? '') + process.env.KEY, {
                        'method': 'POST',
                        'body': compiled.replaceAll('<meta property="og:description" content="An operating system emulator in the browser." />', '<meta property="og:description" content="An operating system emulator in the browser.\n\nWARNING: Experimental build. Subject to live changes." />')
